...
function collectLibrary () {
    ...
    do
        ...
        do
            ...
            unique_lib $lib
            for one in ${g_lib} #lookup the unique symbol links
            do
                if [ -n "$one" ]
                then
                    if [ $bit == 64 ]
                    ...
                    then
                        ...
                        do
                            ...
                            if [ -L ${symbolName} ]; then
                                currentFile=$symbolName
                            fi
                        done
                        if [ $libFlag -eq 1 ]
                        then
...
function unique_lib () {
    ...
    for e in $*
    do
        filename=`echo $(basename $e)`
        dirname=`echo $(dirname $e)`
        f="false"
        for key in ${!uniqueLib[*]}
        do
        ...
        if [ $f = "false" ]; then
            uniqueLib[$filename]=$e
        else
            dir=`echo $(dirname ${uniqueLib[$filename]})`
            if [[ $dirname =~ "nvidia" && ! ${dir} =~ "tls" ]]; then
                uniqueLib[$filename]=$e
            fi
        fi
    done
